// •════════··══════════════════·═══════════════════··══════════════════·═══════════════════··══════════════════·═══════════════════··═══════════•
// ✨λ c3_regex  ι✧21․08․16✦06․40․20․ 🌎η ✧22․12․21․✧22․12․21․✧22․08․19․✧22․04․21․✧21․11․07․
#![allow(dead_code)]
extern crate regex;
use regex::Regex;
use std::error::Error;
...
fn regex_basics() {
    ...
    print!("\n🎡𐡋 Match a 5+ letter word (5 or more) \n");
    ...
    let stl = "aaa ddddaa ccc ddddxx  eee f  g";
    let is_match = re.is_match(stl);
    print!("    is match found? {is_match}\n");

    print!("\n\n🎡𐡋 Capture everything after the first comma in a line (d\\w*) \n");
    let stl = "ignore this but, capture this, and this, and this also";
    let re = Regex::new(r",(.*)").unwrap();
    let res = re.captures(stl);
    match res {                                     // Print string [0] and [1]
        Some(obj) => {
            print!("\nres0: {}", obj.get(0).unwrap().as_str());
            print!("\nres1: {}", obj.get(1).unwrap().as_str());
        },
        None => print!("\nNone"),
    }

    print!("\n\n🎡𐡋 Same as Above but trimming the string  \n");
    let stl = "ignore this, capture this, and this, and this also";
    let re = Regex::new(r",(.*)").unwrap();
    let res = re.captures(stl);
    match res {                                     // Print string [0] and [1]
        Some(obj) => {
    ...
    match res {                                     // Print string [0] and [1]
        Some(obj) => {
            print!("\nres0: {}\n", obj.get(0).unwrap().as_str());
            print!("\nres1: {}\n", obj.get(1).unwrap().as_str());
        },
        None => print!("\nNone\n"),
    }
    ...
    let res = re.captures(stl);                     // re is an Option: Some(stl) or None
    match res {                              
        Some(obj) => print!("\nObj: {:?}\n", obj),  // print entire object
        None => print!("\nNone\n"),
    }
...
fn regex_iterations() {
    ...
    let stl = "ln1: aa dddaaba bb cdc  dddddddxyx eee dddyy f \n\
    ...
    for cap in re.captures_iter(stl) {
        print!("\nMatch:   {:?}\n", cap);
    }
}
