...
//  •═══════··══════════════════·═══════════════════··══════════════════·═══════════════════··══════════════════·═══════════════════··═══════════•
//  ✨λ n0_scratchpad.rs  ι✧21․12․15✦05․21․30․ 🌎η ✧22․11․27․✧22․11․12․✧22․10․22․✧22․08․19․✧22․04․21․✧21․12․15․
#![allow(dead_code)]
use std::iter::zip;
...
/// dot_a1 returns the dot product of 2 arrays (slices) of ints 
fn dot_a1(a1: &[i32], a2: &[i32]) -> i32 {
...
/// dot_a2 returns the dot product of 2 arrays (slices) of ints using map - imperative style
fn dot_a2(a1: &[i32], a2: &[i32]) -> i32 {
...
/// dot_v2; same but for vectors - somewhat functional style
fn dot_v2(v1: Vec<i32>, v2: Vec<i32>) -> i32 {

    let iter = zip( v1.iter(), v2.iter(),);
    iter.map(|(x, y)| x * y).sum()
}


/// dot_v3; same but skiping first 3 tuples - functional style
fn dot_v3(v1: Vec<i32>, v2: Vec<i32>) -> i32 {

    zip( v1.iter(), v2.iter(),).map(|(x, y)| x * y).skip(3).sum()
}


///λ dot_stg returns the dot product of 2 vectors of string-integers as a string 
fn dot_stg(v1: Vec<String>, v2: Vec<String>) -> String {

    // let mut iter = v1.collect
    "ok".to_string()
...
pub fn check_mod() -> Result<(), String> {
    ...
    let aa1 = [1, 2, 3, 2, 2, 2, 2, 2];
    let aa2 = [2, 2, 3, 1, 1, 1, 1, 1];

    let vv1 = aa1.clone().to_vec();
    let vv2 = aa2.clone().to_vec();
