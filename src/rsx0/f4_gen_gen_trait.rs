// •════════··══════════════════·═══════════════════··══════════════════·═══════════════════··═══════════•
// ✨λ f4_gen_gen_trait.rs  generics § traits    ι✧22․06․22✦07․49․56․  🌎η ✧22․07․04․ ✧22․06․28․ ✧22․06․24․
#![allow(dead_code)]
use std::error::Error;
...
///λ max_int find the largest integer in a vector of integers
...
///λ max_char find the largest character in a vector of characters
fn  max_char(vv: &[char]) -> char {

    let mut max_val = &vv[0];                   // but here, max_val is a pointer to char
...
///λ max_val1<T> is a partial generic fnction that returns the first element of the vector
...
///λ max_val2<T> is a generic fnction that finds the largest value in a vector of T
...
//⭐λ max_val3<T> is a generic fnction using the where clause for readability
...
pub fn check_mod() -> Result<(), Box<dyn Error>> {
    ...
    let my_location = "f4_gen_gen_trait::check_mod"; 
    ...
    _gen1.print_ab();
    // _gen1.print_value();         // ⮕> 👎ς error[E0599]: `SomeStruct<{integer}>` doesn't implement `std::fmt::Display`
    _gen2.print_ab();
    // _gen2.print_value();         // ⮕> 👎ς error[E0599]: the method `print_value` exists for struct `SomeStruct<&str>`, but its trait bounds were not satisfied
    // _gen3.print_ab();            // ⮕> 👎ς error[E0599]: the method `print_ab` exists for struct `SomeStruct<[{integer}; 1]>`, but its trait bounds were not satisfied
    // _gen3.print_value();         // ⮕> 👎ς error[E0599]: `SomeStruct<[{integer}; 1]>` doesn't implement `std::fmt::Display`
    _gen4.print_value();
    _gen5.print_value();
